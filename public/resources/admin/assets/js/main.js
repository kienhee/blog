...
(function () {
    ...
    let checkMenuToggler = localStorage.getItem(
        "templateCustomizer-" + templateName + "--LayoutCollapsed"
    );

    // Set initial logo width based on sidebar state
    ...
    menuToggler.forEach((item) => {
        item.addEventListener("click", (event) => {
            ...
            // Update logo width based on sidebar state
            updateLogoWidth();
 ...
 * Clears any existing error message before displaying the new one.
 * Adds the "is-invalid" class to the input for styling.
 *
 * @param {jQuery} $input - The jQuery object representing the input element.
 * @param {string} message - The error message to display below the input.
 */
function showErrorBelowInput($input, message) {
    clearErrorBelowInput($input);
    $("<div>")
...
function clearErrorBelowInput($input) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    tabs.forEach((tab) => {
        tab.addEventListener("shown.bs.tab", function (e) {
            ...
            history.pushState?.(null, "", target); // cập nhật hash
        });
    });
    ...
    if (hash) {
        ...
        if (tab) {
            new bootstrap.Tab(tab).show(); // cách chính thức của Bootstrap để kích hoạt tab
        }
    }
});
