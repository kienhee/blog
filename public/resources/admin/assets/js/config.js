...
let hiddenButtonCustomizer = false; // ẩn nút hiện cài đặt theme
if (hiddenButtonCustomizer) {
  window.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.template-customizer-open-btn').forEach(function(btn) {
      btn.classList.add('d-none');
    });
  });
}

let hiddenDirections = true; // ẩn chức năng RTL | LTR
if (hiddenDirections) {
  window.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.template-customizer-directions').forEach(function(btn) {
      btn.classList.add('d-none');
    });
  });
}
