...
@section('content')
    ...
    <x-breadcrumb parentName="Dự Án" parentLink="dashboard.project.index" childrenName="Thêm Dự Án Mới" />
    <form action="{{ route('dashboard.project.add') }}" method="POST" class="col-xl" enctype="multipart/form-data">
        @csrf
        <div class="card mb-4">
            <x-alert />
            <x-header-table tableName="Thêm Dự Án Mới" link="dashboard.project.index" linkName="Tất Cả Dự Án" />
            <div class="card-body">
                ...
                <div class="d-flex flex-column align-items-center justify-content-center gap-4 mb-3">
                    <img src="{{ asset('images/image-preview.png') }}" alt="Cover"
                        class="d-none rounded cover-img-project img-fluid " id="uploadedAvatar" />
                        ...
                        <select class="form-select" id="categories_select" name="category_project_id">
                            ...
                            @foreach (getAllCategories(2) as $item)
                                <option value="{{ $item->id }}"
                                    {{ old('category_project_id') == $item->id ? 'selected' : false }}>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
